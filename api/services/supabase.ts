...
class SupabaseService {
  ...
  async savePayment(payment: Payment): Promise<{ success: boolean; error?: string }> {
    const data = {
      ...
      updated_at: (payment as any).updatedAt || payment.paidAt,
    };
  ...
  private mapOrder(item: any): Order {
    return {
      ...
      status: item.status,
      playerShareRatio: item.player_share_ratio || 0.5,
  ...
  private mapPlayer(item: any): Player {
    return {
      ...
      shareRatio: item.share_ratio || 0.5,
      wecomUserId: item.wecom_user_id,
      createdAt: item.created_at,
      updatedAt: item.updated_at,
    };
  }
